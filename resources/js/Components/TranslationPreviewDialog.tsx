    ...
    return (
        <Dialog.Root open={open} onOpenChange={handleOpenChange}>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl max-h-[85vh] overflow-auto bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-gray-200 dark:border-slate-800 p-6">
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            <Dialog.Title className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                                <Languages className="h-4 w-4" /> Translation Preview
                            </Dialog.Title>
                            <Dialog.Description className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                                Review the customer-facing translation before sending. Edits override the AI suggestion.
                            </Dialog.Description>
                        </div>
                        <Dialog.Close asChild>
                            <button className="text-gray-400 hover:text-gray-600 dark:text-slate-500 dark:hover:text-slate-200"><X className="h-4 w-4" /></button>
                        </Dialog.Close>
                    </div>

                    {loading && !preview && <p className="text-sm text-gray-500 dark:text-slate-400 py-8 text-center">Loading translation…</p>}
                    {error && <p className="text-sm text-red-600 dark:text-red-400 py-4">{error}</p>}

                    {preview && (
                        <>
                            <div className="text-xs text-gray-500 dark:text-slate-400 mb-3 flex items-center gap-3">
