...
export default function PortalTimeline({ job, token }: Props) {
    return (
        <PortalLayout title="Repair Progress" garageName={job.garage.name}>
            ...
            <Link href={`/portal/${token}`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6">
            ...
            <ol className="relative border-l border-gray-200 space-y-6 pl-5">
                {job.state_transitions.map((t) => (
                    <li key={t.id} className="relative">
                        <div className="absolute -left-[21px] mt-1 h-3 w-3 rounded-full bg-gray-400 border-2 border-white" />
