...
function statusClass(record: ComplianceRecord | null): string {
    if (!record) return 'border-gray-200 bg-gray-50 dark:border-slate-800 dark:bg-slate-800/40';
    const d = daysUntil(record.expires_on);
    if (d < 0) return 'border-red-300 bg-red-50 dark:border-red-900/60 dark:bg-red-950/40';
    if (d <= 30) return 'border-amber-300 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/40';
    return 'border-emerald-300 bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/40';
}

function statusBadge(record: ComplianceRecord | null): { label: string; cls: string } {
    if (!record) return { label: 'Not recorded', cls: 'bg-gray-200 text-gray-600 dark:bg-slate-700 dark:text-slate-300' };
    const d = daysUntil(record.expires_on);
    if (d < 0) return { label: `expired ${Math.abs(d)}d ago`, cls: 'bg-red-200 text-red-800 dark:bg-red-900/60 dark:text-red-200' };
    if (d === 0) return { label: 'expires today', cls: 'bg-amber-200 text-amber-800 dark:bg-amber-900/60 dark:text-amber-200' };
    if (d <= 30) return { label: `${d}d left`, cls: 'bg-amber-200 text-amber-800 dark:bg-amber-900/60 dark:text-amber-200' };
    return { label: `${d}d left`, cls: 'bg-emerald-200 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200' };
}
    ...
    return (
        <CustomerLayout title={vehicle.registration}>
            ...
            <div className="mb-4">
                <Link
                    href="/account"
                    className="inline-flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200"
                >
                ...
                {tab === 'compliance' && (
                    <div className="space-y-3">
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <p className="font-semibold text-gray-900 dark:text-white">{COMPLIANCE_LABELS[type]}</p>
                        ...
                        {complianceHistory.length > 0 && (
                            <details className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-4 mt-4">
                                ...
                                <ul className="mt-3 space-y-2 text-xs text-gray-600 dark:text-slate-400">
                                    {complianceHistory.map((r) => (
                                        <li key={r.id} className="flex items-center gap-2">
                                            ...
                                            <span className="px-1.5 py-0.5 rounded text-[10px] bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 uppercase">
                                                {r.source}
                                            </span>
...
function Detail({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
    return (
        <div>
            <p className="text-xs font-medium uppercase text-gray-500 dark:text-slate-400 tracking-wide">{label}</p>
            <p className={`text-sm text-gray-900 dark:text-white mt-0.5 ${mono ? 'font-mono' : ''}`}>{value}</p>
        </div>
    );
}
