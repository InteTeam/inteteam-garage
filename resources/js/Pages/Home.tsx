...
export default function Home() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased selection:bg-emerald-200/60 dark:selection:bg-emerald-500/40">
            <Head title="InteTeam Garage — repair work, documented." />

            {ssoError && (
                <div className="border-b border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/40 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
                    <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
                        <span>{ssoError}</span>
                        {ssoLogoutUrl && (
                            <a
                                href={ssoLogoutUrl}
                                className="rounded-md bg-amber-900 dark:bg-amber-500 px-3 py-1.5 text-xs font-medium text-amber-50 dark:text-amber-950 hover:bg-amber-800 dark:hover:bg-amber-400"
                            >
            ...
            <header className="sticky top-0 z-40 border-b border-slate-200/60 dark:border-slate-800/60 bg-white/75 dark:bg-slate-950/75 backdrop-blur-xl">
                <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
                    <Link href="/" className="flex items-center gap-2">
                        <span className="relative inline-flex h-7 w-7 items-center justify-center rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900">
                    ...
                    <nav className="hidden items-center gap-6 text-xs font-medium text-slate-600 dark:text-slate-400 sm:flex">
                        ...
                        <a href="#portal" className="transition-colors hover:text-slate-900 dark:hover:text-white">For customers</a>
                    </nav>
                    <div className="flex items-center gap-2">
                        <ThemeToggle />
                        <Button asChild size="sm" className="bg-slate-900 hover:bg-slate-700 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200">
                            <a href="/sign-in">
            ...
            <section className="relative overflow-hidden">
                <div aria-hidden className="pointer-events-none absolute inset-0">
                    <div className="absolute inset-0 home-grid-bg dark:opacity-40" />
                    <div className="absolute -top-32 left-[10%] h-80 w-80 rounded-full bg-emerald-300/40 dark:bg-emerald-500/20 blur-3xl home-blob" />
                    <div className="absolute top-20 right-[5%] h-96 w-96 rounded-full bg-sky-300/40 dark:bg-sky-500/20 blur-3xl home-blob" style={{ animationDelay: '-6s' }} />
                    <div className="absolute -bottom-20 left-[35%] h-72 w-72 rounded-full bg-violet-300/30 dark:bg-violet-500/20 blur-3xl home-blob" style={{ animationDelay: '-12s' }} />
                </div>

                <div className="relative px-4 pb-20 pt-16 sm:pt-24">
                    <div className="mx-auto max-w-3xl text-center">
                        <div className="home-fade-in">
                            <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 px-3 py-1 text-xs font-medium text-slate-700 dark:text-slate-300 shadow-sm backdrop-blur">
                        ...
                        <p
                            className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-slate-600 dark:text-slate-400 sm:text-lg home-fade-up"
                        ...
                        >
                            <Button asChild size="lg" className="group h-12 rounded-full bg-slate-900 px-6 text-sm shadow-lg shadow-slate-900/10 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200 dark:shadow-slate-900/40">
                                <a href="/sign-in">
                                    Sign in
                                    <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                                </a>
                            </Button>
                            <Button asChild size="lg" variant="outline" className="h-12 rounded-full border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/60 dark:text-slate-100 px-6 text-sm backdrop-blur hover:bg-white dark:hover:bg-slate-900">
                                <a href="#how-it-works">
                                    See how it works
                        ...
                        <p className="mt-4 text-xs text-slate-500 dark:text-slate-500 home-fade-up" style={{ animationDelay: '0.55s' }}>
            ...
            <section aria-label="Trust strip" className="relative border-y border-slate-200/70 dark:border-slate-800/70 bg-white/60 dark:bg-slate-900/60 py-5 backdrop-blur">
                <div className="overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
                    <div className="flex w-max gap-12 home-marquee">
                        {[...TRUST, ...TRUST].map((label, i) => (
                            <span key={`${label}-${i}`} className="inline-flex shrink-0 items-center gap-2 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">
                ...
                <div className="mx-auto max-w-6xl">
                    <Reveal>
                        <div className="max-w-2xl">
                            <span className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
                    ...
                    <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {FEATURES.map(({ icon: Icon, title, body, accent }, i) => (
                            <Reveal key={title} delay={i * 80}>
                                <div className={cn(
                                    'home-card-glow group relative h-full overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6',
                                )}>
                                    <div className={cn(
                                    ...
                                    <div className="relative">
                                        <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg shadow-slate-900/10 transition-transform group-hover:scale-110">
            ...
            <section id="how-it-works" className="relative overflow-hidden px-4 py-20 sm:py-28">
                <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 home-grid-bg opacity-50 dark:opacity-25" />
                <div className="mx-auto max-w-6xl">
                    <Reveal>
                        <div className="max-w-2xl">
                            <span className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
                    ...
                    <ol className="relative mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div aria-hidden className="pointer-events-none absolute left-6 right-6 top-12 hidden h-px bg-gradient-to-r from-transparent via-slate-300 dark:via-slate-700 to-transparent lg:block" />
                        {STEPS.map(({ icon: Icon, title, body }, i) => (
                            <Reveal key={title} delay={i * 120}>
                                <li className="relative rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 transition-shadow hover:shadow-lg dark:hover:shadow-slate-900/60">
                                    <div className="flex items-center justify-between">
                                        <div className="relative inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-900 to-slate-700 dark:from-white dark:to-slate-300 text-white dark:text-slate-900 shadow-lg">
                                            <Icon className="h-5 w-5" />
                                        </div>
                                        <span className="rounded-full bg-emerald-50 dark:bg-emerald-900/40 px-2.5 py-0.5 text-[10px] font-semibold tracking-wider text-emerald-700 dark:text-emerald-300">
                                            STEP {i + 1}
                                        </span>
                                    </div>
                                    <h3 className="mt-5 text-base font-semibold text-slate-900 dark:text-white">{title}</h3>
            ...
            <section className="px-4 py-20 sm:py-28">
                <Reveal>
                    <div className="relative mx-auto max-w-4xl overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-white via-slate-50 to-emerald-50/40 dark:from-slate-900 dark:via-slate-900 dark:to-emerald-950/40 p-10 text-center shadow-xl shadow-slate-900/[0.04] dark:shadow-slate-950/40 sm:p-14">
                        <div aria-hidden className="pointer-events-none absolute -right-20 -top-20 h-64 w-64 rounded-full bg-emerald-300/30 dark:bg-emerald-500/20 blur-3xl home-blob" />
                        <div aria-hidden className="pointer-events-none absolute -bottom-20 -left-20 h-64 w-64 rounded-full bg-sky-300/30 dark:bg-sky-500/20 blur-3xl home-blob" style={{ animationDelay: '-7s' }} />
                        <div className="relative">
                            ...
                            <div className="mt-7 flex justify-center">
                                <Button asChild size="lg" className="group h-12 rounded-full bg-slate-900 px-7 text-sm shadow-lg shadow-slate-900/10 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200">
                                    <a href="/sign-in">
                                        Sign in
                                        <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                                    </a>
            ...
            <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-8">
                <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 text-xs text-slate-500 dark:text-slate-400 sm:flex-row">
                    ...
                    <div className="flex items-center gap-4">
                        <a href="https://inte.team" className="transition-colors hover:text-slate-900 dark:hover:text-white" target="_blank" rel="noreferrer">
                            Inte.Team
                        </a>
                        <a href="/sign-in" className="transition-colors hover:text-slate-900 dark:hover:text-white">
                            Sign in
                        </a>
                    </div>
                </div>
...
function PortalLine({ label, price, status }: { label: string; price: string; status: 'ok' | 'new' }) {
    return (
        <li className={cn(
            ...
            status === 'new' ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/40' : 'border-slate-200 dark:border-slate-700'
        )}>
            <span className={cn('truncate', status === 'new' ? 'text-emerald-900 dark:text-emerald-200' : 'text-slate-700 dark:text-slate-300')}>{label}</span>
            <span className={cn('font-semibold', status === 'new' ? 'text-emerald-900 dark:text-emerald-200' : 'text-slate-900 dark:text-white')}>{price}</span>
        </li>
    );
}
...
function HeroMock() {
    return (
        <div className="relative">
            ...
            <div className="relative grid grid-cols-1 gap-3 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/80 p-3 shadow-2xl shadow-slate-900/[0.08] dark:shadow-slate-950/60 backdrop-blur-xl md:grid-cols-3">
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Active jobs</span>
                        <span className="rounded-full bg-emerald-50 dark:bg-emerald-900/40 px-2 py-0.5 text-[10px] font-semibold text-emerald-700 dark:text-emerald-300">+3 today</span>
                    </div>
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 md:col-span-2">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Latest job</span>
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/40 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:text-amber-300">
                            <span className="h-1.5 w-1.5 rounded-full bg-amber-500" />
                            Awaiting approval
                        </span>
                    </div>
                    <p className="mt-3 text-base font-semibold">BD23 KLM · Ford Focus</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">3 stages documented · 9 photos · estimate sent 12 min ago</p>
                    <div className="mt-3 grid grid-cols-3 gap-2">
                        {['Pre-inspection', 'Disassembly', 'Fault found'].map((stage, i) => (
                            <div key={stage} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-2">
