...
- [x] Translation preview UI side-by-side — backend contract shipped 2026-06-08 (Q4 + Q5): `preview-translation` returns translations + `from_locale`/`to_locale`/`configured_from_locale`/`auto_detected_override`; `confirm-translation` accepts `confirmations[]` with `{id, translated_text, llm_raw_text}` per line item; service does diff-detection to set `translation_edited_by_mechanic_id` audit FK; atomic transaction flips `preview_confirmed_at` only if all rows persist. React side-by-side editor shipped 2026-06-10 — `resources/js/Components/TranslationPreviewDialog.tsx` opens from `RepairJobs/Show.tsx` "Preview & Send" button when `current_estimate.preview_confirmed_at` is null, fetches the LLM preview via axios, lets the mechanic edit each translation, retry per item, and posts confirm → send in one click.
...
**Status: complete 2026-06-10.** All 5 questions (Q1–Q5) resolved by Piotr; backend implementation of every Phase 6 acceptance item shipped 2026-06-08. UI follow-ups closed 2026-06-10: (a) `Components/TranslationPreviewDialog.tsx` — side-by-side editor with retry-per-item, posts `confirm-translation` then `send` in one click; trigger button wired into `RepairJobs/Show.tsx` gated by `current_estimate.preview_confirmed_at`; (b) `JobStageController::updateNotes()` + `UpdateJobStageNotesRequest` + route `PATCH /jobs/{job}/stages/{stage}/notes` + `Components/StageNotesEditor.tsx` rendered inline on the job page; (c) `MechanicController::create()` + `edit()` Inertia handlers added (forms were unreachable before), `Mechanics/Form.tsx` now renders `locale` select + `channel_toggle_allowed` checkbox with help copy. See memory `garage-phase6-deferred.md` for the locked decisions.
...
- [x] Integration test: simulate full customer journey via portal API (estimate → approve → handover → collected) — `tests/Feature/Portal/FullCustomerJourneyTest.php` (1 test, 16 assertions). Portal view → approve 2 line items → mechanic transitions awaiting_approval → approved → completed → awaiting_collection → customer handover with both items accepted → payment webhook → mechanic transitions to collected. Asserts final state + the 3 expected ApprovalEvent rows (`LINE_ITEM_APPROVED` x2, `HANDOVER_SUBMITTED`, `PAYMENT_CONFIRMED`).
