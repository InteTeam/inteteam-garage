...
| POST | `/jobs/{job}/estimates/{estimate}/line-items` | `LineItemController@store` | Mechanic-side draft add (audit 7 / B1). `EstimateService::addLineItem` seals on both `sent_at !== null` and `hasCustomerResponse()` — either rejection surfaces as `assertSessionHasErrors('description')` via `ValidationException::withMessages`. Authorized via `LineItemPolicy::create` (admin-only) + explicit garage match. Frontend: `AddLineItemForm` inside `EstimatePanel.tsx`, only rendered when `!sent_at` and no item has been actioned. |
...
| `EstimateService` | CRUD on `Estimate`. `update()` **throws** when `Estimate::hasCustomerResponse()` is true — must create a new revision instead. `addLineItem()` (audit 7 / B1) **throws** when `sent_at !== null` OR `hasCustomerResponse()` — same revision-or-bust rule but seals at the earlier of "sent to customer" and "first customer response". `createForJob($job)` is the race-safe revision creator (locks parent job row + `max+1` inside `DB::transaction`); `EstimateController::store` delegates to it. `markSent()` wraps the state transition + sent timestamp in a transaction. `guardSendable()` throws on cross-locale send without confirmation. `confirmTranslation()` atomically persists per-line-item LLM-raw + edited text + editor mechanic FK and flips `Estimate.preview_confirmed_at`. |
...
- **Estimate immutability after send or customer response.** `EstimateService::update()` throws `RuntimeException` if `Estimate::hasCustomerResponse()` is true (any line item approved/declined). `EstimateService::addLineItem()` (audit 7 / B1) seals on the wider window: `sent_at !== null` OR `hasCustomerResponse()`, because the customer may already be reading the estimate in the portal. To change scope after either point, create a new `Estimate` row with `revision_number = previous + 1` — `EstimateController::store()` already does this, and `ScopeChangeService` is the canonical way for state `approved → scope_change`.
...
- **`JobStage.sort_order` is derived server-side, never accepted from the client.** `JobStageController::store` computes `sort_order = array_search($name, JobStage::STAGES, true)` after `Rule::in(JobStage::STAGES)` validates the name (audit 7 / F2). `StoreJobStageRequest` deliberately omits the `sort_order` rule so a hand-crafted POST cannot reorder the canonical pipeline or duplicate-position another stage's row. The frontend `AddStageControl` in `Components/StageNotesEditor.tsx` receives the `stagesAvailableToAdd: string[]` prop from `JobController::show` (audit 7 / F3) so the React side no longer mirrors `JobStage::STAGES` as a separate constant.
...
- **`sessions.user_id` is `varchar(36)`, not `bigint` (migration 030).** Laravel's default migration sizes the column for bigint mechanic IDs. The customer guard's `Customer` uses a 26-char ULID, and the DB session driver writes it to `user_id` at end-of-request — the original schema threw `Data truncated for column 'user_id'` and silently 500'd the response after the controller had already finished. If a third guard is added with another ID shape, widen further or move to per-guard tables. See `docs/database/migrations/030_widen_sessions_user_id.md`.

- **`auth:customer` failure must NOT default to `route('login')`.** `bootstrap/app.php` `redirectGuestsTo` routes unauth `/account/*` to `customer.login` and everything else to `login`. Without the per-path branch, an unauth GET on `/account` redirects to mechanic `/login`, which bounces through SSO and the mechanic callback (rejects non-mechanics with `redirect(route('home'))`) — but if the home redirect ever bounces back to `/login` again it loops. The test `CustomerDashboardTest::test_unauthenticated_redirected_to_customer_login_not_mechanic` asserts the **exact** target — a regression that drops the per-path branch fails the test.

- **Customer-portal sign-out clears SSO session too.** `CustomerSsoLoginController::logout` and `SsoLoginController::logout` both `Inertia::location` to `{SSO_PUBLIC_URL}/logout?redirect_to={garage home}` after clearing the local session. The SSO endpoint (`logoutWithRedirect`) requires `https://` in production but explicitly allows `http://localhost:*` when `app()->environment('local')` — needed for the localhost dev story since browsers cannot redirect to plain-http hosts on https origins. Without the SSO clear, the next login button auto-passes the same SSO user back in.

- **Customer auth via SSO can succeed without a CRM link.** The callback creates a `Customer` row regardless of CRM match. `Customer::isLinkedToCrm()` is the single source of truth — `false` triggers the dashboard banner and **every detail endpoint short-circuits to 404** via `whereRaw('1 = 0')` in the closure form of `when()`. Do not branch on `crm_customer_id === null` in callers; call `isLinkedToCrm()`. Tests in `CustomerDashboardTest`, `CustomerVehicleAndJobTest`, `CustomerTransactionControllerTest`.

- **Customer line-item actions have a state gate at the service layer.** `LineItemDecisionService::ACTIONABLE_STATES = [STATE_AWAITING_APPROVAL, STATE_CUSTOMER_QUERY]`. The service throws `RuntimeException` outside that window — defense-in-depth so a hand-crafted POST cannot mutate an `awaiting_collection` or `collected` job's audit trail. `Customer\LineItemController` pre-checks the same and aborts 409 (separates "wrong moment on your own job" from 404 "not your job"). Same gate applies to the signed-token portal — both call into the same service.

- **`actor_id` on customer audit events is the `Customer.id` (SSO portal) or `SignedPortalToken.token` (signed portal).** Both `Customer\LineItemController` and `Portal\PortalLineItemController` pass `actorId` explicitly to `LineItemDecisionService`. Skipping it falls back to `'unknown'` (the `ApprovalEventService::record` default) which makes the audit log dishonest. Tests assert `actor_id = $customer->id` and `actor_id = $token`.
